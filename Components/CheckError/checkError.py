...
class checkError:
    ...
    def run(self,text,line,rule,message , errorLen): #madde 1 için pyright çağırıldı ve çıktılar parse'ye gönderildi.    
        self.text = text
        self.line = line
        return self.parse(rule,message,errorLen)
    
    def parse(self, rule,message,errorLen):                                      # pyright çıktılarındaki ilk hata mesajı ve dosyada karşılık gelen satır alındı.
        if len(errorLen) > 9 :
            ...
            try:    
                errorLine = self.text[self.line].replace("\r","") # text değişkenin içinden hatalı satırı, errorLine içerisine kaydedildi.
                if errorLine[0:1] == "\t":
                ...
                try:   
                    if (rule == "reportGeneralTypeIssues"): # from io import BytesIOxxxxx --> gibi hatalarda bu döngüye girecektir.    
                        if errorLine.count("from") > 0:
                            split_errorLine = errorLine.split()      
                            returnDataString = ("%s paketi Kurulu Değil veya Yanlış Yazılmış, Lütfen %s paketini kontrol ediniz.."  %(line1 ,line1))
                            self.returnData.append(returnDataString)
                            return self.returnData
                        else:
                    ...
                    elif (rule == "reportMissingImports"):    # import xPandasx // from xxioxx import BytesIO  --> gibi hatalarda bu döngüye girecektir.    
                    ...
                    elif (message == "Girintisizlik beklenmiyor"):    # import xPandasx // from xxioxx import BytesIO  --> gibi hatalarda bu döngüye girecektir.     
                        returnDataString = "girintihatasi" + errorLine  
                        self.returnData.append(returnDataString)              
                        return self.returnData
                    
                    elif (message == "Girintili kod bloğu bekleniyor"):    # import xPandasx // from xxioxx import BytesIO  --> gibi hatalarda bu döngüye girecektir.     
                        returnDataString = "girintihatasi" + errorLine   
                        self.returnData.append(returnDataString)              
                        return self.returnData
    ...
    def is_user_string(self, dataList):
        ...
        for i in range(len(dataList)):
            if dataList[i] not in self.listed_python_data:        # hatalı satırımızı ve Python_Words tokenlerini karşılaştırıyoruz 
                try:
    ...
    def token(self,data):       #ilk olarak hatalı satırı tokenlerine ayırmak için kullandık.
        ...
        for token in g:         # gelen data verisi tokenlerine ayrılarak liste içeriği null karakterden arındırıldı.
            if token.string != '' and token.string != "\n":               
                if token.string[0] == '"' and token.string[-1] == '"' and len(token.string) > 2: # String değerlerde ki " işaretini ayrımak için ayrı bir koşul bloğu yazıldı.
                    tokenarray.append('"')
        ...
        return tokenarray[1:]   # ilk değer her zaman utf-8 olduğu için çıkan sonucun 1. indisinden itibaren liste geri döndürüldü
    ...
    def kNN(self,errorLine):
      # tüm datasetler okundu ve liste olarak data değişkenine kaydedildi.
        ...
        for i in data:                        # textdistance ye göre data değişkenlerinin hatalı koda göre uzaklıkları hesaplandı ve bir listeye atandı
            distance.append(textdist.ratcliff_obershelp.distance(i,errorLine)) #jaccard,ratcliff_obershelp,sqrt_ncd,tversky
        _,data= zip(*sorted(zip(distance,data), reverse=False)) # hata mesajları ve uzaklıkları ilişkilendirilerek knn algoritması için uzunluklar küçükten büyüğe doğru sıralandı
                                                                # ve sıralı data değişkeni tekrar data değişkenine aktarıldı.                                                        
        knnList = []
        for i in range(5):  # kNN algoritması için sıralanan data listesinin ilk 5 üyesi \n ve soldaki boşluk ve tab karakterlerinden temizlendi.
        ...
        errorList = []
        for i in knnList:
            aln = alignment.needle(errorData,i) # Hatalı datamız ile kNN çıktılarının Gen alignment algorintmasını çalıştırdık.
            identity = aln[0]
            score = aln[1]
        ...
        result_list = prepare_output.my_alliagment_fonc(selectedList , errorList , self.listed_python_data , OriginalStrings)      
